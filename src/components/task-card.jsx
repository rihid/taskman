  ...
  return (
    ...
    >
      ...
      <div className="flex items-center">
        <span className="inline-block capitalize text-sm rounded-full px-2 py-1 text-center bg-slate-100 text-foreground">{task.category}</span>
      </div>
      ...
      <div className="flex justify-between items-center gap-2 my-2">
        <span className={`
          inline-block capitalize text-sm rounded-full px-2 py-1 text-center
          ${task.status === 'todo' ? 'bg-amber-500 text-background' : ''}
          ${task.status === 'doing' ? 'bg-green-500 text-background' : ''}
          ${task.status === 'done' ? 'bg-blue-500 text-background' : ''}
        `}>
          {task.status}
        </span>
        <button
